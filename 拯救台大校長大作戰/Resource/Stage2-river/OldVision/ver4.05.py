...
def Opening_Trailer():
	...
	stage2 = obstacle(1280,180,192,96,-20)
	ZhoushanRiver = obstacle (1280+500,265,192,96,-20)
...
### game exit while loop
	...
	while not gameExit:
		...
		set_Obstacle(bus,busImg)
		bus.x += bus.movespeed
		bus.y += bus.movespeed_y

		set_Obstacle(cat,catImg)		
		cat.x += cat.movespeed
		cat.y += cat.movespeed_y

		set_Obstacle(dog,dogImg)
		dog.x+= dog.movespeed
		dog.y+= dog.movespeed_y

		set_Obstacle(fish,fishImg)
		fish.x+= fish.movespeed
		fish.y+= fish.movespeed_y

		set_Obstacle(dino,dinoImg)
		dino.x += dino.movespeed
		dino.y += dino.movespeed_y

		set_Obstacle(rainbowcat,rainbowcatImg)
		...
		if flash == False: 
			if boatImgNum!=2:                                  
				...
				set_MainCharacter(boat,boatImg0)
				if frame % 7 == 0:
					boatImgNum = 0
		...
		## set time on right upper field
		set_Text(now_time,"Right")
		time_running=float(pygame.time.get_ticks()/1000)
		now_time.content="%-10.1f"%(pygame.time.get_ticks()/1000)
